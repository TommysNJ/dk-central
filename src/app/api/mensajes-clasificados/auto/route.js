...
export async function POST(req) {
  ...
  try {
    ...
    // --------------------------------------------------------------------
    // 🚀 CASO C: No clasificado → clasificar (FastAPI warm) + fallback spawn
    // --------------------------------------------------------------------
    ...
    try {
      ...
      if (!r.ok) {
        ...
        throw new Error(`FastAPI status ${r.status}: ${txt.slice(0, 200)}`);
      }

      result = await r.json();
    } catch (e) {
      console.warn("⚠️ FastAPI no disponible, usando fallback spawn:", e?.message || e);

      // ✅ 2) Fallback: spawn como lo tenías
